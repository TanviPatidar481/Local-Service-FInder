...
const BusinessVerifyEmail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const finalData = {
        user_id: userId,
        ...JSON.parse(localStorage.getItem("businessSignup") || "{}"),
        ...JSON.parse(localStorage.getItem("businessBasicInfo") || "{}"),
        ...JSON.parse(localStorage.getItem("businessLocation") || "{}"),
        ...JSON.parse(localStorage.getItem("businessCategory") || "{}"),
      };
      await api.post("/business/create", finalData);
    ...
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.left}>
          ...
          <h1 style={s.heading}>Verify your<br/><span style={s.headingGreen}>business</span></h1>
          <p style={s.subtext}>We've sent a verification code to your registered business email. Enter the code below to continue.</p>
          <div style={s.pill}>
            ...
            <p style={s.pillText}>Verification email sent to: <strong>{email}</strong></p>
          </div>
          <div style={s.pill}>
            <div style={s.pillIcon}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            ...
            <p style={s.pillText}>This helps us keep your business account secure.</p>
          </div>
        </div>
        ...
        <div style={s.right}>
          ...
          <p style={s.tag}>BUSINESS VERIFICATION</p>
          <h2 style={s.formTitle}>Enter verification <span style={s.formTitleGreen}>code</span></h2>
          <p style={s.formSub}>Type the 6-digit code sent to your business email.</p>
          <form onSubmit={handleSubmit} style={s.form}>
            <div style={s.field}>
              <label style={s.label}>Verification Code <span style={s.req}>*</span></label>
              <div style={s.inputWrap}>
                <svg style={s.ico} width="16" height="16" viewBox="0 0 24 24" fill="none">
                  <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="#2e7d32" strokeWidth="1.8" strokeLinejoin="round"/>
                </svg>
                <input style={{ ...s.input, letterSpacing:"0.1em" }} type="text" maxLength={6} value={verificationCode} onChange={(e) => setVerificationCode(e.target.value)} placeholder="Enter 6-digit code"/>
              </div>
            </div>
            <button type="submit" disabled={!isValid} style={{ ...s.btn, ...(isValid ? {} : s.btnDisabled) }}>
